...
class AdService {
  ...
  Future<void> initialize() async {
    ...
    // Check if user is premium using project's keys
  ...
  Future<void> _checkPremiumStatus() async {
    try {
      ...
      // Syncing with project's Pro status keys
  ...
  /// Update user premium status (call when user buys a package or earns time)
  Future<void> setPremiumStatus(bool isPremium) async {
    _isPremiumUser = isPremium;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('is_premium_user', isPremium); // Internal key
    if (isPremium) {
       debugPrint('[AdService] 💎 Premium Status Updated — Stopping Ad initialization');
    }
  }
  ...
  void showInterstitial({VoidCallback? onComplete}) {
    ...
    if (!_isInitialized || !_isInterstitialLoaded) {
      debugPrint('[AdService] ⚠️ Interstitial not ready, calling onComplete');
      onComplete?.call();
      return;
    }
  ...
  void showRewarded({
    ...
    if (!isRewardedReady()) {
      debugPrint('[AdService] ⚠️ Rewarded ad not ready for show');
      onFailed?.call();
      return;
    }
